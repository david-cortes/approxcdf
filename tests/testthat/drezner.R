library("testthat")
library("mvtnorm")
context("Adaptations from Drezner")
...
cdf2d_tvpack <- function(x1, x2, rho) {
    return(as.numeric(pmvnorm(upper=c(x1,x2), corr=cor2d(rho), algorithm=TVPACK(abseps=1e-14))))
}
...
test_that("CDF 2D Fast", {
    ...
    for (i in 1:1000) {
        ...
        cdf1 <- .Call(R_norm_cdf_2d_fast, x1, x2, rho)
...
test_that("CDF 3D Fast", {
    ...
    for (i in 1:50000) {
        ...
        if (det(R) <= 0) next
        cdf1 <- .Call(R_norm_cdf_3d_fast, x1, x2, x3, rho[1], rho[2], rho[3])
